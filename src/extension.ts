...
import { setUpAgentHelper, offerRoutingSetup, setUpCompanion } from './setup';
...
import { readSurfaceState, SurfaceState } from './surfaces';
...
const prefixCommand = 'codexRepoCompanion.bridge.setAssignments';
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (!git) { return; }
  const output = vscode.window.createOutputChannel('Codex Repo Companion');
  const status = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 15);
  status.command = 'codexRepoCompanion.assignRepository';
  ...
  function repositoryNames() {
    const aliases = readRepositoryAliases(vscode.workspace.getConfiguration('codexRepoCompanion').get('repositoryAliases'));
    return [...aliases, ...(vscode.workspace.workspaceFolders?.map(folder => ({ fsPath: folder.uri.fsPath, name: folder.name })) ?? [])];
  }

  function displayFor(key: string, folders = repositoryNames()) {
    const assignment = assignments[key];
    const mode = effectiveMode(modes[key], assignment);
    if (customLabels[key]) {
      return { label: customLabels[key], tooltip: mode === 'pinned' ? 'Custom label (pinned)' : 'Custom label (manual correction; follows newer discussion)' };
    }
    ...
    if (hideRedundantLabel(roots.map(root => root.fsPath), git!.repositories.filter(repo => repo.rootUri.scheme === 'file').map(repo => repo.rootUri.fsPath),
      mode === 'auto' && !manualTimes[key], vscode.workspace.getConfiguration('codexRepoCompanion').get('hideRedundantRepositoryLabels', true))) {
  ...
  async function saveManualChoice(key: string) {
    ...
    modes[key] = 'pinned';
    const pin = vscode.workspace.getConfiguration('codexRepoCompanion').get('pinManualLabels', true);
  ...
  async function updateAutoScope(key: string): Promise<boolean> {
    ...
    const detect = vscode.workspace.getConfiguration('codexRepoCompanion').get('detectChatFocus', false);
    ...
    const choices = [
      { report: agentReport, source: 'agent' as const },
    ...
    if (manualTimes[key] !== manualTime || (customLabels[key] && !manualTime) || effectiveMode(modes[key], assignments[key]) !== 'auto' || disposed) { return false; }
  ...
  function activeSurface() {
    ...
    return state?.surfaces.find(surface => surface.id === state.activeId);
  }

  function chat() {
    const surface = activeSurface();
    if (surface?.kind === 'sidebar') {
      return surface.key ? { key: surface.key, surfaceId: surface.id, kind: surface.kind,
        uri: undefined, tab: undefined } : undefined;
    }
    const tab = vscode.window.tabGroups.activeTabGroup.activeTab;
    const input = tab?.input;
    if (input instanceof vscode.TabInputCustom && input.viewType === conversationViewType) {
      const panel = surfaceState?.surfaces.find(item => item.kind === 'panel' && item.uri === input.uri.toString());
      if (panel?.routeKnown) {
        return panel.key ? { key: panel.key, surfaceId: undefined, kind: 'panel' as const, uri: input.uri, tab } : undefined;
      }
    }
    return editorChat();
  }
  ...
  async function syncPrefixes() {
    ...
    bridgeAvailable = commands.includes(prefixCommand);
    if (commands.includes('codexRepoCompanion.bridge.getState')) {
      const state = readSurfaceState(await vscode.commands.executeCommand('codexRepoCompanion.bridge.getState'));
      if (state && (!surfaceState || state.revision >= surfaceState.revision)) { surfaceState = state; }
    } else { surfaceState = undefined; }
    if (bridgeAvailable) {
      const enabled = vscode.workspace.getConfiguration('codexRepoCompanion').get('showTabPrefix', true);
      const folders = repositoryNames();
  ...
  function showStatus() {
    ...
    if (!active) {
      const surface = activeSurface();
      if (surface) {
        status.text = '$(repo) No saved chat selected';
        status.tooltip = 'Open a conversation or send the first message in a new chat, then assign its repository.';
        status.command = 'codexRepoCompanion.assignRepository';
        status.show();
      } else if (isLauncher()) {
        ...
        status.command = 'codexRepoCompanion.openSavedChat';
        status.show();
      } else { status.hide(); }
      return;
    }
    status.command = 'codexRepoCompanion.scopeMenu';
  ...
  function report(error: unknown, notify = false) {
    ...
    const silent = vscode.workspace.getConfiguration('codexRepoCompanion').get('silentMode', true);
    if (notify || (!silent && !warned.has(message))) {
      warned.add(message);
      void vscode.window.showWarningMessage(`Codex Repo Companion: ${message}`);
    }
  }
  ...
  function refresh(force = false) {
    ...
    queue = queue.then(async () => {
      if (disposed) { return; }
      await syncPrefixes();
      if (changed && revision === generation && surfaceState) {
        const input = activeTab?.input;
        const uri = input instanceof vscode.TabInputCustom ? input.uri.toString() : null;
        const state = readSurfaceState(await vscode.commands.executeCommand('codexRepoCompanion.bridge.editorActivated', uri));
        if (state && (!surfaceState || state.revision >= surfaceState.revision)) { surfaceState = state; }
      }
      const active = chat();
      if (active) {
  ...
  async function assign(uri?: vscode.Uri, repositoryUri?: vscode.Uri) {
    ...
    if (!current?.key && !uri && activeSurface()) {
      throw new Error('No saved conversation is selected in the focused Codex view. Open a chat or send its first message, then try again.');
    }
    ...
    if (!current?.key) { throw new Error('Focus a saved Codex conversation first. Direct sidebar assignment requires the v0.2 local Codex bridge; without it, use Open Saved Codex Chat.'); }
    const folders = repositoryNames();
    ...
    if (picks.length <= 1 && !repositoryUri) {
      await setCustomLabel(vscode.Uri.from({ scheme: 'openai-codex', authority: 'route', path: '/' + current.key }));
      return;
    }
    ...
    await saveManualChoice(current.key);
    ...
    await queue;
    if (!bridgeAvailable) {
      void vscode.window.showInformationMessage('Repository saved. Tab prefixes require the local Codex bridge; see README setup.');
    }
  }
  ...
  async function setCustomLabel(uri?: vscode.Uri, suppliedLabel?: string) {
    ...
    const value = suppliedLabel ?? await vscode.window.showInputBox({
      title: 'Custom chat label', prompt: vscode.workspace.getConfiguration('codexRepoCompanion').get('pinManualLabels', true) ? 'Display label only. Stays pinned until changed manually.' : 'Display label only. Newer discussion can update it.',
  ...
  function historyUri(value: unknown): vscode.Uri {
    const key = historyContextKey(value);
    if (!key) { throw new Error('Right-click a saved local Codex chat to choose its repository.'); }
  ...
  const commands: [string, (...args: any[]) => unknown][] = [
    ...
    ['surfaceChanged', (value: unknown) => {
      const state = readSurfaceState(value);
      if (!state || (surfaceState && state.revision <= surfaceState.revision)) { return; }
      const before = activeSurface();
      const after = state.surfaces.find(surface => surface.id === state.activeId);
      const contextChanged = before?.id !== after?.id || before?.key !== after?.key;
      surfaceState = state;
      if (contextChanged) { generation++; }
      refresh();
    }],
    ...
    ['toggleStar', async (uri?: vscode.Uri) => {
      await syncPrefixes();
      const key = uri instanceof vscode.Uri ? conversationKey(uri) : chat()?.key;
      if (!key || !historyContextKey({ webviewSection: 'repoCompanionChat', repoCompanionConversationKey: key })) { throw new Error('Open or right-click a saved local Codex chat first.'); }
    ...
    ['starHistoryChat', (value: unknown) => vscode.commands.executeCommand('codexRepoCompanion.toggleStar', historyUri(value))],
    ...
    ['associateLabelRepository', async () => {
      await syncPrefixes();
      const key = chat()?.key;
      if (!key || !customLabels[key]) { throw new Error('Open a chat with a custom label first.'); }
    ...
    ['customHistoryLabel', (value: unknown, label?: string) => setCustomLabel(historyUri(value), label)],
    ['chooseHistoryRepository', (value: unknown, repository?: vscode.Uri) => assign(historyUri(value), repository)],
    ['autoHistoryScope', (value: unknown) => vscode.commands.executeCommand('codexRepoCompanion.useAutomaticScope', historyUri(value))],
    ['clearHistoryLabels', (value: unknown) => vscode.commands.executeCommand('codexRepoCompanion.clearRepository', historyUri(value))],
    ['scopeMenu', async () => {
      await syncPrefixes();
      const active = chat();
      if (!active) { await assign(); return; }
      ...
      const picked = await vscode.window.showQuickPick([
        ...
        { label: 'Pin current scope', description: 'Keep the current labels and primary repository', action: 'pin' },
        { label: 'Choose repositories...', description: 'Choose the primary repository, then any related repositories', action: 'choose' },
        { label: 'Clear labels', description: 'Pause automatic assignment for this chat', action: 'none' },
    ...
    ['diagnostics', async () => {
      ...
      const result = { vscode: vscode.version, codex: vscode.extensions.getExtension('openai.chatgpt')?.packageJSON.version,
        tabBridge: bridgeAvailable, repositories: git!.repositories.length, assignments: Object.keys(assignments).length,
        sidebarBridge: surfaceState?.protocol === 2, activeKey: chat()?.key ?? null,
  ...
  for (const [name, action] of commands) {
    context.subscriptions.push(vscode.commands.registerCommand(`codexRepoCompanion.${name}`, (...args) => Promise.resolve().then(() => action(...args)).catch(error => report(error, true))));
  }
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('codexRepoCompanion')) { refresh(); }
    }),
  ...
  for (const kind of ['reports', 'corrections'] as const) {
    const reportsDirectory = path.join(home, 'repo-companion', kind);
  ...
  try {
    const sessionWatcher = watch(path.join(home, 'sessions'), { recursive: true }, (_event, filename) => {
      ...
      if (!vscode.workspace.getConfiguration('codexRepoCompanion').get('detectChatFocus', false)) { return; }
      sessions.invalidate();
  ...
  try { for (const item of await readRecentConversations(home)) { knownKeys.add('local/' + item.id); } }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') { report(error); } }
  scheduleScopeRefresh();
  refresh(true);
  monitorChatLabels(context);
  const invitationTimer = setTimeout(() => {
    if (!disposed) { void offerRoutingSetup(context).catch(error => report(error)); }
  }, 2000);
  context.subscriptions.push({ dispose() { clearTimeout(invitationTimer); } });
}
