...
export function readColours(value: unknown, kind: 'chat' | 'repository'): Record<string, string> {
  ...
  for (const [key, valueColour] of Object.entries(value)) {
    const colour = kind === 'repository' && valueColour === 'none' ? 'none' : normaliseColour(valueColour);
...
export function colourLab(colour: string): number[] {
...
export function blendColours(colours: string[]): string | undefined {
  ...
  for (const colour of valid) {
    ...
    for (let index = 0; index < 3; index++) sums[index] += lab[index];
  }
  return labColour(sums.map(value => value / valid.length));
}

export function labColour([L, a, b]: number[]): string {
