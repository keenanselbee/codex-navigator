...
Version 1.0.9 provides display labels and stars: it does not select repositories, move
...
and Codex updates may need a newer Companion version. Nothing patches automatically.
...
when chat labels need attention; **Open Extensions** helps you check for updates.
...
a measured CPU/memory benchmark. See [architecture](docs/architecture.md).
...
npm test
npm run package
code --install-extension .\dist\codex-repo-companion-1.0.9.vsix
```
...
older patches while preserving their originals. It does not automatically patch updates.
Version 1.0.9 bundles the existing v0.12.0 display patch; existing patched users
...
to preserve reports or corrections. See [architecture](docs/architecture.md) and
[verification status](docs/verification.md) for details.
